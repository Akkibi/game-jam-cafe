...
const MultBar = () => {
  ...
  useGSAP(
    () => {
      ...
      waveing.current = gsap.timeline({ paused: false, smoothChildTiming: true }).fromTo(
        wave,
        {
      ...
      waveing2.current = gsap.timeline({ paused: false, smoothChildTiming: true }).fromTo(
        wave2,
        {
          backgroundPositionY: '50%',
        },
  ...
  return (
    <>
      <div className="absolute top-5 left-1/2 -translate-x-1/2 w-[70vh] h-[6vh]">
        <BarBackground color={currentBackground} className="absolute inset-0 w-full h-full" />
        <div
