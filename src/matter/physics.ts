...
export class PhysicsEngine {
  ...
  public addCircle(position: THREE.Vector3, radius: number, moving: boolean = false): Matter.Body {
    const newCoord = mapCoords(position, true);
  ...
  public update(deltaTime: number): void {
    ...
    const newVelocityX = Math.min(
      Math.max(
        bodyVelocity.x +
          speed.x * (isCurrentTouch ? 2 : 1) * (0.8 + useStore.getState().caffeineLvl * 0.02),
        -5,
      ),
      5,
    );
