...
export class GameEngine {
  ...
  static getInstance(
    ...
    if (!this._instance) this._instance = new GameEngine(physics, seedManager, soundManager, scene);
  ...
  private getNextBlockData(oldBlock: GameBlock): (typeof Blocks)[0] | undefined {
    ...
    if (this.isEndlessPhase) {
      ...
      if (availableBlocks.length > 0) {
        const nextBlockData = availableBlocks[Math.floor(Math.random() * availableBlocks.length)];
